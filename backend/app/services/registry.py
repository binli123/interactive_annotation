...
class ObjectRegistry:
    ...
    def scan(self, folder: Path | None = None) -> list[ObjectRecord]:
        ...
        def inspect_h5ad(object_path: Path) -> tuple[int | None, int | None, bool, str | None]:
            try:
        ...
        def register_object(lineage_dir: Path, object_path: Path) -> None:
            manifest_path = lineage_dir / "recluster_manifest.json"
            manifest = json.loads(manifest_path.read_text()) if manifest_path.exists() else {}
            inferred_n_cells, inferred_n_genes, is_valid, validation_error = inspect_h5ad(object_path)
            lineage_name = str(
        ...
        if lineage_dirs:
            ...
            for object_path in sorted(lineage_root.glob("*.h5ad")):
                register_object(lineage_root, object_path)
